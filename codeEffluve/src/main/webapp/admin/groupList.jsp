...
<div class="pagest">
   <!-- 왼쪽  -->
    <% if(startPage > 1) { %>
        <a class ="arrow" 
        href="/codeEffluve/admin/admin.jsp?menu=group&cp=<%=startPage - 1%>&pageSize=<%=pageSize %><%
    ...
    %>
        <a class="page-num <%=active%>"
           href="/codeEffluve/admin/admin.jsp?menu=group&cp=<%=i%>&pageSize=<%=pageSize %><%
                if(searchId != null && !searchId.equals("")){ %>&searchId=<%=searchId%><% } %>">
           <%=i%>
        </a>
    <% } %>
    ...
    <% if(endPage < totalPage) { %>
        <a class ="arrow" 
        href="/codeEffluve/admin/admin.jsp?menu=group&cp=<%=endPage + 1%>&pageSize=<%=pageSize %><%
            if(searchId != null && !searchId.equals("")){ %>&searchId=<%=searchId%><% } %>">
            ▶
        </a>
    <% } %>
